...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.updateWinner = exports.createWinnerFirst = exports.getWinner = exports.driveCarApi = exports.stopCarApi = exports.startCarApi = exports.updateCar = exports.showCars = exports.deleteCar = exports.addCarApi = exports.winners = exports.engine = exports.garage = void 0;
...
const getWinner = async (url, id, time) => {
    ...
    if (result.status === 404) {
        ...
        (0, exports.createWinnerFirst)(exports.winners, id, time, 1);
    }
    else {
        console.log(data);
        (0, exports.updateWinner)(url, time, data);
    }
    // return { id, data };
};
...
const svg_1 = __webpack_require__(/*! ./svg */ "./modules/svg.ts");
...
function createCarsElement() {
    (0, api_1.showCars)(api_1.garage).then(value => {
        for (const key of value) {
            ...
            elem.innerHTML = (0, svg_1.svg)(key.color, key.name, key.id);
...
function createCarElement(car) {
    const elem = document.createElement('div');
    elem.innerHTML = (0, svg_1.svg)(car.color, car.name, car.id);
    elem.classList.add('car-element');
    const cars = document.querySelectorAll('.car');
    if (cars.length % 7 === 0) {
        const garageList = document.createElement('div');
    ...
    raceButton.addEventListener('click', (event) => {
        ...
        for (const key in Array.from(garageList.children)) {
            Array.from((garageList.children)[key].children).forEach(elem => {
                if (elem.tagName === 'svg') {
                    const result = (0, api_1.startCarApi)(api_1.engine, +elem.id).then((value) => (0, api_1.driveCarApi)(api_1.engine, value.id, value.data));
                    promiseArr.push(result);
                }
            });
        }
        Promise.any(promiseArr).then(value => (0, api_1.getWinner)(api_1.winners, value.id, +(value.propertyCar.distance / value.propertyCar.velocity / 1000).toFixed(2)));
    });
}
exports.race = race;
...
function reset() {
    resetButton.addEventListener('click', (event) => {
        ...
        for (const key in Array.from(garageList.children)) {
            Array.from((garageList.children)[key].children).forEach(elem => {
                if (elem.tagName === 'svg') {
                    (0, api_1.stopCarApi)(api_1.engine, +elem.id).then(() => {
...
function updateCarColorAndText(button, value) {
    button.parentElement.parentElement.innerHTML = (0, svg_1.svg)(value.color, value.name, value.id);
    findSelectCar();
    (0, removeCar_1.removeCar)();
    (0, driveCar_1.startCar)();
}
